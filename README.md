...
データはすべてPCローカル（ローカルの世界）で完結するため、社内情報やプライベートなナレッジベースも極めて安全に管理・構築できます。

### 🧠 AIネイティブなローカル知識循環エコシステム
* **openManidoc (デスクトップ)**: 人間が知識を俯瞰（マインドマップ）し、整理・アノテーション（マークダウン、画像枠線注釈、テーマ生成）するための知識構築・閲覧ツール。
* **manidocMCP**: 外部のAIデスクトップ等から、ローカルに蓄積された安全なデータにアクセスするための **MCP（Model Context Protocol）サーバー**。曖昧な検索に頼るRAG（検索拡張生成）とは異なり、Markdownで構造化されたツリーから直接、横断的な全文検索を行って正確な情報を引き出します。これにより、AIのハルシネーション（もっともらしい嘘）を防ぎ、社内の確実なソースに基づいて正確に答えさせることができます。また、AIにデータを直接操作させてナレッジを自律構築することも可能です。
